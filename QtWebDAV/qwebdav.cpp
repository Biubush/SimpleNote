...
QNetworkReply *QWebdav::lock(const QString &path, qint64 secs, const QString& owner, QWebdavLockScope scope, QWebdavLockDepth depth, const QString& locktoken)
{
    ...
    if (secs < 0) {
        ...
        seconds = QString("Second-") + QString::number(secs);
    }
    req.setRawHeader("Timeout", seconds.toUtf8());

    if (!locktoken.isEmpty()) {
        req.setRawHeader("If", "(<" + locktoken.toUtf8() + ">)");
    }
...
QNetworkRequest QWebdav::buildRequest()
{
    ...
    if (isSSL()) {
        ...
        QString headerData = QString("Basic ") + QString::fromLatin1(data);
